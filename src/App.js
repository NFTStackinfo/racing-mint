...
function App() {
  ...
  const secondCountdown = ({
    ...
    if (days === 0 && hours === 0 && seconds === 0 && milliseconds === 0 &&
        minutes === 10) {
    ...
    if (completed) {
      ...
      return <>
        ...
        {walletConnected && notSelected && !publicMintActive ? (
            <>
              <p className="text">Unfortunately you have not been selected to
                mint.</p>
              {blockchain.account &&
                  <p className="yellow-text">Wallet Address - ${truncateText(
                      blockchain.account)}</p>}
              <Button
        ...
        totalSupply <= maxTotalSupply ? (
            <>
              ...
              {!publicMintActive &&
                  <p className="text">Congrats! You have been selected to
                    mint.</p>}
              {publicMintActive &&
                  <p className="text">Supply: {totalSupply} / {maxTotalSupply}</p>}
              {blockchain.account &&
                  <p className="yellow-text">Wallet Address - ${truncateText(
                      blockchain.account)}</p>}
      ...
      // Render a countdown
      return <div>
        <p className="text">Mint starts
          in {days} days {hours} hours {minutes} minutes {seconds} seconds.</p>
      </div>;
    }
  };
      ...
      return <div>
        {blockchain.registerMessage ? (
            <>
              ...
              <p className="text">Check back
                in {days} days {hours} hours {minutes} minutes {seconds} seconds
                to see if you were selected to mint.</p>
            </>
        ) : (
            <>
              <h2>REGISTER FOR RAFFLE</h2>
              <p className="text">The registration is free and registering is
                only available with a metamask wallet. <br/> Registration period
                ends
                in {days} days {hours} hours {minutes} minutes {seconds} seconds.
              </p>
              <p className="yellow-text">You need to have at least 0.18 ETH +
                gas fee to participate in raffle.</p>
